...
export function DashboardPage() {
  ...
  useEffect(() => {
    if (totalFranchises > 0 && readyCount === totalFranchises) {
      setCountdown(5)
    } else {
      setCountdown(null)
    }
  }, [readyCount, totalFranchises])
  ...
  const handleSetReady = async () => {
    ...
    setIsReadying(true)
    useUiStore.getState().setLoading(true, 'Maça Hazırlanıyor...')
    try {
  ...
  return (
    <div className="space-y-4 pt-4">
      ...
      <div className="bg-[#00152b] border border-[#005c99] rounded-xl p-4 flex flex-col items-center justify-center text-center">
          ...
          {franchise?.is_ready && countdown === null && (
            <p className="text-accent text-xs font-bold mt-4 uppercase animate-pulse">Diğer menajerlerin hazır olması bekleniyor...</p>
          )}
          {countdown !== null && (
            <p className="text-emerald-400 text-sm font-bold mt-4 uppercase animate-pulse">
              {countdown > 0 ? `Maç Başlıyor... ${countdown}` : `Maç Yükleniyor...`}
            </p>
          )}
        </div>
